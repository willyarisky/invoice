...
<div class="space-y-6">
    <div class="grid gap-4 md:grid-cols-3">
        <div class="rounded-lg border border-stone-200 bg-white px-6 py-5 shadow-sm">
        ...
        <div class="rounded-lg border border-stone-200 bg-white px-6 py-5 shadow-sm">
            ...
            <p class="mt-2 text-3xl font-semibold text-stone-900">{{ \App\Models\Setting::formatMoney((float) ($metrics['outstanding'] ?? 0)) }}</p>
        </div>
    </div>
        ...
        <div class="rounded-lg border border-stone-200 bg-white px-6 py-5 shadow-sm">
            ...
            <div class="mt-4 grid gap-4 sm:grid-cols-3 lg:grid-cols-1">
                @foreach ($statusSummary as $label => $count)
                    <div class="rounded-lg border border-stone-100 bg-stone-50 px-4 py-4 text-center">
                    ...
                    <tbody class="divide-y divide-stone-100">
                        @foreach ($recentInvoices as $invoice)
                            ...
                            <tr>
                                ...
                                <td class="px-6 py-3">
                                    <span class="inline-flex items-center rounded-lg px-3 py-1 text-xs font-semibold {{ $badge }}">{{ ucfirst($status) }}</span>
                                </td>
                                <td class="px-6 py-3 text-right font-semibold">{{ \App\Models\Setting::formatMoney((float) ($invoice['total'] ?? 0), $invoice['currency'] ?? null) }}</td>
                            </tr>
                        @empty
                            <tr>
