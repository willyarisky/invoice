...
<div class="grid gap-8 lg:grid-cols-[240px_1fr]">
    ...
    <div class="space-y-6">
        <div class="flex items-center justify-between gap-3 sm:gap-4 lg:justify-between">
            <div>
                <h1 class="text-2xl font-semibold text-stone-900">Available categories</h1>
            </div>
            <button type="button" class="inline-flex h-9 w-9 items-center justify-center rounded-xl bg-stone-900 text-white hover:bg-stone-800 sm:h-auto sm:w-auto sm:px-4 sm:py-2 sm:text-sm sm:font-semibold" x-on:click="openAdd()" aria-label="Add category">
                            ...
                            <td class="px-4 py-3 text-right">
                                <div class="flex justify-end gap-4">
                                    <button
                                        ...
                                        data-action="{{ $category['edit_action'] ?? '' }}"
                                        data-name="{{ $category['name'] ?? '' }}"
                                        x-on:click="openEdit($el.dataset.action, $el.dataset.name)"
                                    >
                                        Edit
                                    </button>
...
<div id="category-modal" class="fixed inset-0 z-50" x-cloak x-show="addOpen" x-on:click.self="closeAdd()">
