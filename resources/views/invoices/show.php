...
<div class="space-y-6">
    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between print:hidden">
        ...
        <div class="flex gap-3">
            <button type="button" onclick="window.print()" class="rounded-full border border-stone-200 px-4 py-2 text-sm font-semibold text-stone-700 hover:bg-stone-50 print:hidden">Print invoice</button>
            <a href="{{ route('invoices.create') }}" class="rounded-full bg-stone-800 px-5 py-2 text-sm font-semibold text-white hover:bg-stone-700">Duplicate</a>
        </div>
    </div>
    <div class="rounded-3xl border border-stone-200 bg-white p-6 shadow-sm">
        <div class="flex flex-col gap-4 border-b border-stone-100 pb-6 md:flex-row md:items-center md:justify-between">
            ...
            <div class="flex items-center gap-3">
                <span class="inline-flex items-center rounded-full border px-4 py-1 text-sm font-semibold {{ $badge }}">
                    {{ ucfirst($status) }}
                </span>
                <div class="text-right">
                    <p class="text-xs uppercase tracking-widest text-stone-400">Balance</p>
                    <p class="text-3xl font-semibold text-stone-900">${{ number_format((float) ($invoice['total'] ?? 0), 2) }}</p>
                </div>
            </div>
        </div>
                ...
                <tbody class="divide-y divide-stone-100">
                    @foreach ($items as $item)
                        <tr>
                            ...
                            <td class="px-4 py-3 text-right font-semibold">${{ number_format((float) ($item['subtotal'] ?? 0), 2) }}</td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
        ...
        <div class="mt-6 flex flex-col items-end gap-2 text-right">
            ...
            <div class="text-4xl font-semibold text-stone-900">${{ number_format((float) ($invoice['total'] ?? 0), 2) }}</div>
        </div>
    </div>
</div>
@endsection
