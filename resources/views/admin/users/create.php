...
<div class="space-y-6">
    <div class="rounded-lg border border-stone-200 bg-white px-6 py-6 shadow-sm">
        ...
        <form method="POST" action="{{ route('admin.users.store') }}" class="mt-6 grid gap-4 lg:grid-cols-2">
            ...
            <label class="flex flex-col text-sm font-medium text-stone-700">
                Temporary password
                <input type="password" name="password" class="mt-1 rounded-lg border border-stone-200 bg-white px-4 py-2 text-stone-700">
            ...
            <label class="flex flex-col text-sm font-medium text-stone-700">
                Confirm password
                <input type="password" name="password_confirmation" class="mt-1 rounded-lg border border-stone-200 bg-white px-4 py-2 text-stone-700">
            ...
            <label class="flex items-center gap-3 text-sm text-stone-600 lg:col-span-2">
                <input type="checkbox" name="send_verification" value="1" class="h-4 w-4 rounded-lg border-stone-300 text-stone-700" @if (!empty($old['send_verification'])) checked @endif>
                        ...
                        <tr>
                            ...
                            <td class="px-6 py-3">
                                @if (!empty($user['email_verified_at']))
                                    <span class="inline-flex items-center rounded-lg bg-emerald-100 px-3 py-1 text-xs font-semibold text-emerald-700">Verified</span>
                                @else
                                    <span class="inline-flex items-center rounded-lg bg-amber-100 px-3 py-1 text-xs font-semibold text-amber-700">Pending</span>
                                @endif
                            </td>
