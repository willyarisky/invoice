@layout('layouts.app', ['title' => 'Edit Vendor'])

@section('content')
<div class="space-y-6">
    <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div>
            <h1 class="text-2xl font-semibold text-stone-900">Edit vendor</h1>
        </div>
    ...
    <div class="rounded-xl border border-stone-200 bg-white px-6 py-6 shadow-sm">
        ...
        <form method="POST" action="{{ route('vendors.update', ['vendor' => $vendor['id']]) }}" class="mt-6 grid gap-4 lg:grid-cols-2">
            ...
            <label class="flex flex-col text-sm font-medium text-stone-700">
                Phone number
                <input type="text" name="phone" value="{{ $values['phone'] ?? '' }}" class="mt-1 rounded-xl border border-stone-200 bg-white px-4 py-2 text-stone-700">
            ...
            <label class="flex flex-col text-sm font-medium text-stone-700 lg:col-span-2">
                Address
                <textarea name="address" rows="3" class="mt-1 rounded-xl border border-stone-200 bg-white px-4 py-2 text-stone-700">{{ $values['address'] ?? '' }}</textarea>
